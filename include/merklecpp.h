...
static inline uint64_t deserialise_uint64_t(const std::vector<uint8_t> &bytes,
                                            size_t &index) {
  uint64_t r = 0;
...
template <size_t SIZE> struct HashT {
  /// Holds the hash bytes
  uint8_t bytes[SIZE];

  /// @brief Constructs a Hash with all bytes set to zero
  HashT<SIZE>() { std::fill(bytes, bytes + SIZE, 0); }
  ...
  size_t serialised_size() const { return SIZE; }
  ...
  std::string to_string(size_t num_bytes = SIZE, bool lower_case = true) const {
    ...
    for (size_t i = 0; i < num_bytes; i++)
      snprintf(const_cast<char *>(r.data() + 2 * i), num_chars + 1 - 2 * i,
               lower_case ? "%02x" : "%02X", bytes[i]);
    return r;
  }
...
template <size_t HASH_SIZE,
          void HASH_FUNCTION(const HashT<HASH_SIZE> &l,
                             const HashT<HASH_SIZE> &r, HashT<HASH_SIZE> &out)>
class PathT {
public:
  ...
  /// @param elements
  /// @param max_index
  PathT(const HashT<HASH_SIZE> &leaf, size_t leaf_index,
        std::list<Element> &&elements, size_t max_index)
      : _leaf(leaf), _leaf_index(leaf_index), _max_index(max_index),
        elements(elements) {}
  ...
  PathT(const std::vector<uint8_t> &bytes) { deserialise(bytes); }
...
protected:
  ...
  struct Node {
    ...
    static Node *copy_node(const Node *from,
                           std::vector<Node *> *leaf_nodes = nullptr,
                           size_t *num_flushed = nullptr, size_t min_index = 0,
                           size_t max_index = SIZE_MAX, size_t indent = 0) {
      if (from == nullptr)
        return nullptr;
      ...
      r->dirty = from->dirty;
      r->left = copy_node(from->left, leaf_nodes, num_flushed, min_index,
                          max_index, indent + 1);
      r->right = copy_node(from->right, leaf_nodes, num_flushed, min_index,
                           max_index, indent + 1);
...
public:
  ...
  TreeT(TreeT &&other)
      : leaf_nodes(std::move(other.leaf_nodes)),
        ...
        _root(std::move(other._root)), num_flushed(other.num_flushed),
  ...
  TreeT(const std::vector<uint8_t> &bytes) { deserialise(bytes); }
  ...
  TreeT(const Hash &root) { insert(root); }
  ...
  bool invariant() { return _root ? _root->invariant() : true; }

  /// @brief Inserts a hash into the tree
  /// @param hash Hash to insert
  void insert(const uint8_t *hash) { insert(Hash(hash)); }
  ...
  void flush_to(size_t index) {
    ...
    leaf_nodes.erase(leaf_nodes.begin(),
                     leaf_nodes.begin() + num_newly_flushed);
  ...
  void retract_to(size_t index) {
    ...
    Node *new_leaf_node = walk_to(index, true, [this](Node *&n, bool go_right) {
      ...
      if (go_left && n->right) {
        ...
        *n = *old_left;
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - after elimination: " << std::endl
                                       << to_string(TRACE_HASH_SIZE)
                                       << std::endl;);
        return false;
      } else
        return true;
  ...
  Tree &operator=(const Tree &other) {
    ...
    _root = Node::copy_node(other._root, &leaf_nodes, &to_skip,
                            other.min_index(), other.max_index());
  ...
  inline Node *walk_to(size_t index, bool update,
  ...
  std::shared_ptr<Path> path(size_t index) {
    ...
    return std::make_shared<Path>(leaf_node(index)->hash, index,
                                  std::move(elements), max_index());
  }
  ...
  std::shared_ptr<Path> past_path(size_t index, size_t as_of) {
    MERKLECPP_TRACE(MERKLECPP_TOUT << "> past_path from " << index << " as of "
                                   << as_of << std::endl;);
    ...
    for (uint8_t height = _root->height; height > 1;) {
      ...
      if (!fork_node && go_right_i != go_right_a) {
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - split at "
                                       << cur_i->hash.to_string(TRACE_HASH_SIZE)
                                       << std::endl;);
        fork_node = cur_i;
      }
  ...
  void serialise(std::vector<uint8_t> &bytes) {
    ...
    serialise_uint64_t(leaf_nodes.size() + uninserted_leaf_nodes.size(), bytes);
  ...
  void serialise(size_t from, size_t to, std::vector<uint8_t> &bytes) {
    ...
    if ((from < min_index() || max_index() < from) ||
  ...
  const Hash &operator[](size_t index) const { return leaf(index); }
  ...
  size_t min_index() const { return num_flushed; }
  ...
  bool empty() const { return num_leaves() == 0; }
...
protected:
  ...
  void insert_leaves(bool complete = false) {
    if (!uninserted_leaf_nodes.empty()) {
      MERKLECPP_TRACE(MERKLECPP_TOUT << "* insert_leaves " << leaf_nodes.size()
                                     << " +" << uninserted_leaf_nodes.size()
                                     << std::endl;);
...
static inline void sha256_compress_openssl(const HashT<32> &l,
                                           const HashT<32> &r, HashT<32> &out) {
...
/// @param out Output node hash
/// @note Some versions of OpenSSL may not provide SHA256_Transform.
static inline void sha256_openssl(const merkle::HashT<32> &l,
...
/// use only.
static inline void sha256_compress_mbedtls(const HashT<32> &l,
                                           const HashT<32> &r, HashT<32> &out) {
  unsigned char block[32 * 2];
  memcpy(&block[0], l.bytes, 32);
  memcpy(&block[32], r.bytes, 32);
...
/// @param r Right node hash
/// @param out Output node hash
static inline void sha256_mbedtls(const merkle::HashT<32> &l,
                                  const merkle::HashT<32> &r,
                                  merkle::HashT<32> &out) {
  uint8_t block[32 * 2];
  memcpy(&block[0], l.bytes, 32);
