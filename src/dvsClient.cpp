...
*/
void Data::getTimestamp() { block->timestamp = time(NULL); }
...
void DvsClient::encryptBlock() {
  ...
  unsigned char encrypted[2024], *encBlock = encrypted,
                                 *privateKey = (unsigned char *)pKey.c_str();
  RSA *rsa = createRSA(privateKey);
  int enc_len = RSA_private_encrypt(temp.length(), longBlock1, encBlock, rsa,
                                    RSA_PKCS1_PADDING);
  if (enc_len == -1) {
