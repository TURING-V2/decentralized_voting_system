...
void DvsNode::process_request(
  ...
  case http::verb::post: {
    ...
    if (req.target().find("pubKeyHash") != std::string::npos) {
      if (b2->CheckpubKey(req.target().substr(req.target().find("h=") + 2,
                                              std::string::npos)) == 1) {
        if (b2->decrypt(req.target().substr(req.target().find("d=") + 2,
                                            req.target().find("pu") - 11)) ==
            0) {
        ...
        send_bad_response(http::status::bad_request, "WRONG PUBLIC KEY");
    }
    break;
  }
  case http::verb::get: {
    if (req.target().find("MerkleRoot") != std::string::npos) {
      std::string url =
          std::string{req.target().substr(0, req.target().find("merk") - 12)};
      ...
      std::string url =
          std::string{req.target().substr(0, req.target().find("ledger") - 7)};
      cpr::Response r = cpr::Post(
          cpr::Url{url}, cpr::Multipart{{"key", "large value"},
...
int Blockchain::decrypt(beast::string_view target) {
  ...
  int result =
      RSA_public_decrypt(len, encrypted, decBlock, rsa, RSA_PKCS1_PADDING);
  if (result == -1) {
...
bool DvsNode::getMerkleRoot() {
  ...
  for (std::map<std::string, int>::iterator it2 = m.begin(); it2 != m.end();
       ++it2) {
